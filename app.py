...
class ProcessRequest(BaseModel):
    import_id: str
    source_type: str = Field(pattern='^(MARGEM|GESTAO_PRODUCAO)def db_conn():
    if not DATABASE_URL:
...
def _mirror_batch_sent(batch_id: str):
    with db_conn() as conn:
        row = conn.execute(
...
def _mirror_send_batch(payload: ProcessRequest | MirrorContext, dataset: str, headers: list[str],
    ...
    if not force and _mirror_batch_sent(batch_id):
        return {'status': 'SKIPPED', 'reason': 'batch já enviado', 'batch_id': batch_id}
...
def _mirror_headers_and_keys(import_id: str, dataset: str):
    with db_conn() as conn:
        rows = conn.execute(
            """
    ...
    header_map = {int(col): header for col, header in rows}
    ...
    for idx in range(1, max_col + 1):
        ...
        headers.append(str(header) if header else f'COL_{idx}')

        if header:
            raw_key = str(header)
            if raw_key in used:
                raw_key = f'{raw_key} [{col_letter(idx)}]'
        else:
            raw_key = col_letter(idx)
...
def _mirror_core_dataset(context: MirrorContext, dataset: str, table_name: str,
                         replay_job_id: str, force: bool):
    headers, raw_keys = _mirror_headers_and_keys(context.import_id, dataset)
    if not headers:
        return {'sent_batches': 0, 'skipped_batches': 0, 'sent_rows': 0, 'error_batches': 0}
    ...
    while True:
        ...
        for source_row, raw_data in records:
            raw = raw_data or {}
            out_rows.append([raw.get(key) for key in raw_keys])

        last_source_row = records[-1][0]
        start_row = records[0][0]
        is_last = len(records) < MIRROR_BATCH_SIZE

        try:
            result = _mirror_send_batch(
                context, dataset, headers, out_rows,
        ...
        _update_mirror_job(
            replay_job_id,
            status='PROCESSING',
            message=f'{context.source_type} / {dataset}: lote {batch_number - 1}',
            sent_batches_delta=1 if result.get('status') != 'SKIPPED' else 0,
            skipped_batches_delta=1 if result.get('status') == 'SKIPPED' else 0,
            total_batches_delta=1,
            total_rows_delta=len(out_rows),
            sent_rows_delta=len(out_rows) if result.get('status') != 'SKIPPED' else 0
        )
...
def _raw_excel_header(import_id: str, dataset: str, header_row: int):
    with db_conn() as conn:
        row = conn.execute(
            """
            select row_data
    ...
    for idx in range(1, max_col + 1):
        ...
        headers.append(str(value) if value not in (None, '') else f'COL_{idx}')
        raw_keys.append(letter)

    return headers, raw_keys


def _mirror_raw_dataset(context: MirrorContext, dataset: str, header_row: int,
                        first_data_row: int, replay_job_id: str, force: bool):
    headers, raw_keys = _raw_excel_header(context.import_id, dataset, header_row)
    if not headers:
        return {'sent_batches': 0, 'skipped_batches': 0, 'sent_rows': 0, 'error_batches': 0}
...
def _update_mirror_job(job_id: str, *, status=None, progress=None, message=None,
    ...
    if progress is not None:
        fields.append('progress=%s'); params.append(progress)
    if message is not None:
        fields.append('message=%s'); params.append(message)

    if total_batches_delta:
        fields.append('total_batches=total_batches+%s'); params.append(total_batches_delta)
    if sent_batches_delta:
        fields.append('sent_batches=sent_batches+%s'); params.append(sent_batches_delta)
    if skipped_batches_delta:
        fields.append('skipped_batches=skipped_batches+%s'); params.append(skipped_batches_delta)
    if error_batches_delta:
        fields.append('error_batches=error_batches+%s'); params.append(error_batches_delta)
    if total_rows_delta:
        fields.append('total_rows=total_rows+%s'); params.append(total_rows_delta)
    if sent_rows_delta:
        fields.append('sent_rows=sent_rows+%s'); params.append(sent_rows_delta)
    if finished:
...
def replay_active_mirror_job(job_id: str, requested_source: str, force: bool):
    try:
        if not mirror_configured():
            raise RuntimeError('MIRROR_URL ou MIRROR_TOKEN não configurado no Render.')

        _update_mirror_job(
            job_id, status='PROCESSING', progress=2,
            message='Localizando importações ativas...'
        )
        ...
        if not active:
            raise RuntimeError('Nenhuma importação ativa encontrada para o filtro solicitado.')

        total_sources = len(active)

        for source_index, (import_id, source_type) in enumerate(active, start=1):
            context = MirrorContext(import_id=import_id, source_type=source_type)

            if source_type == 'MARGEM':
                _mirror_core_dataset(context, 'BD', 'core.realizado', job_id, force)
                _mirror_core_dataset(context, 'BD_Meta', 'core.meta', job_id, force)
                _mirror_raw_dataset(context, 'TD_Meta', 1, 2, job_id, force)
                _mirror_raw_dataset(context, 'Apoio', 1, 2, job_id, force)
                _mirror_raw_dataset(context, 'Tabela de Preço', 2, 3, job_id, force)
            else:
                _mirror_core_dataset(
                    context, 'Gestao da ProdV6', 'core.gestao_producao',
                    job_id, force
                )

            progress = min(98, 5 + (source_index / total_sources) * 90)
            _update_mirror_job(
                job_id,
                status='PROCESSING',
                progress=progress,
                message=f'{source_type} concluído no espelhamento.'
            )
...
def replay_active_mirror(payload: MirrorReplayRequest, background_tasks: BackgroundTasks):
    if not mirror_configured():
        raise HTTPException(
            503,
            'Espelhamento não configurado: verifique MIRROR_URL e MIRROR_TOKEN no Render.'
        )
...
def mirror_job_status(job_id: str):
    ...
    if not row:
        raise HTTPException(404, 'Job de espelhamento não encontrado.')
...
def mirror_summary():
    ...
    return {
        ...
        'batches': [
            {'status': r[0], 'dataset': r[1], 'batches': r[2], 'rows': r[3]}
            for r in rows
        ]
    }
...
class MirrorReplayRequest(BaseModel):
    source_type: str = Field(default='ALL', pattern='^(ALL|MARGEM|GESTAO_PRODUCAO)def db_conn():
    if not DATABASE_URL:
        raise RuntimeError('DATABASE_URL não configurada.')
    # As cargas XLSX usam COPY de dezenas de milhares de linhas.
    # O Supabase deste projeto usa statement_timeout=2min por padrão,
